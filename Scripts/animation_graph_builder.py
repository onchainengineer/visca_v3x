...
class AnimationGraphBuilder:
    ...
    def add_compare_two_variables_transition(
        ...
        condition_logical_prim_path: str = transition_prim_path + "/ConditionLogical"
        assert logical_operator == "AND" or logical_operator == "OR"
        if logical_operator == "AND":
            condition_logical = ag.AnimGraphSchema.ConditionAND.Define(
                self._stage, condition_logical_prim_path
            )
        elif logical_operator == "OR":
            condition_logical = ag.AnimGraphSchema.ConditionOR.Define(
                self._stage, condition_logical_prim_path
            )
            
        condition_logical: Usd.Prim = self._stage.GetPrimAtPath(condition_logical_prim_path)

        self.connect_nodes(
            condition_a_prim_path, condition_logical_prim_path, "inputs:condition0"
        )
        self.connect_nodes(
            condition_b_prim_path, condition_logical_prim_path, "inputs:condition1"
        )

        self.connect_nodes(
            condition_logical_prim_path, transition_prim_path, "inputs:condition"
        )

        return transition_prim_path

    def add_compare_variable_with_time_fraction_crossed_transition(
        self,
        from_state_prim_path: str,
        to_state_prim_path: str,
        variable: str,
        ...
        # Create a ConditionTimeFractionCrossed
        condition_time_crossed_prim_path: str = transition_prim_path + "/ConditionTimeCrossed"
        condition_time_crossed = ag.AnimGraphSchema.ConditionTimeFractionCrossed.Define(
            self._stage, condition_time_crossed_prim_path
        )
        condition_time_crossed: Usd.Prim = self._stage.GetPrimAtPath(condition_time_crossed_prim_path)
        condition_time_crossed.GetAttribute("inputs:fraction").Set(fraction)

        # Create logical condition
        condition_logical_prim_path: str = transition_prim_path + "/ConditionLogical" + logical_operator
        assert logical_operator == "AND" or logical_operator == "OR"
        if logical_operator == "AND":
            condition_logical = ag.AnimGraphSchema.ConditionAND.Define(
                self._stage, condition_logical_prim_path
        ...
        self.connect_nodes(
            condition_prim_path, condition_logical_prim_path, "inputs:condition0"
        )
        self.connect_nodes(
            condition_time_crossed_prim_path, condition_logical_prim_path, "inputs:condition1"
        )
