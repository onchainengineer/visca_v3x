...
class AnimationGraphBuilderBehaviorScript(BehaviorScript):
    ...
    def on_init(self) -> None:
        ...
        self._anim_graph_start_clip_prim_path: str = (
            self._main_state_machine_prim_path + "/GesturesMainState/GesturesStateMachine/Start/AnimationClip"
        )
        self._anim_graph_end_clip_prim_path: str = (
            self._main_state_machine_prim_path + "/GesturesMainState/GesturesStateMachine/End/AnimationClip"
        )
        ...
        self._gestures: List[Gesture] = [
            ...
            for child_prim in self._stage.GetPrimAtPath(self._config.gesture_scope_prim_path).GetAllChildren()
        ...
        self._postures = []
        for scope_prim in self._stage.GetPrimAtPath(self._config.posture_scope_prim_path).GetAllChildren():
        ...
        self._builder.connect_nodes(
            main_state_machine_prim_path, self._config.anim_graph_prim_path + "/Blend", "inputs:pose0"
        )
        ...
        def gestures_state_machine(gestures_main_state_prim_path: str) -> None:
            ...
            for gesture in self._gestures:
                ...
                self._builder.add_time_fraction_crossed_transition(state_prim_path, end_state_prim_path)
            ...
            # Connect the start node with the end node. This is needed when the user sets an invalid gesture state. This will play back the one finger start node animation and then once finished transition to the end node, which will reset the gesture variable to "none".
            self._builder.add_time_fraction_crossed_transition(start_state_prim_path, end_state_prim_path)
        ...
        def postures_state_machines(main_state_machine_prim_path: str) -> None:
            ...
            for posture_index, posture in enumerate(self._postures):
                ...
                is_start_state: bool = posture.name == self._start_state_posture_name or posture_index == 0
                ...
                self._builder.connect_nodes(
                    variant_state_machine_prim_path, posture_main_state_prim_path, "inputs:pose" 
                )
                ...
                for variant_index, outer_prim_variant in enumerate(posture.prim_variants):
                    outer_variant_state_prim_path: str = self._builder.add_state_with_animation(
                        variant_state_machine_prim_path,
        ...
        skel_root_prim.GetRelationship("animationGraph").SetTargets([self._config.anim_graph_prim_path]) 
        ...
        animation_graph_prim.GetRelationship("skel:skeleton").SetTargets([self._config.skeleton_prim_path])
        ...
        blinking_darting_animation_clip_node_prim_path: str = self._builder.add_animation_clip_node(
        ...
        blend_node_prim_path: str = self._builder.add_blend_node(animation_graph_prim_path, "Blend")
        filter_node_prim_path: str = self._builder.add_filter_node(animation_graph_prim_path, "Filter")

        read_joint_positions_variable_node_prim_path: str = self._builder.add_variable_node(
            animation_graph_prim_path, self._joint_positions_variable_name
        )
        read_joint_rotations_variable_node_prim_path: str = self._builder.add_variable_node(
            animation_graph_prim_path, self._joint_rotations_variable_name
        )
        read_root_position_displacement_variable_node_prim_path: str = self._builder.add_variable_node(
            animation_graph_prim_path,
            self._root_position_displacement_variable_name,
        )
        read_root_rotation_displacement_variable_node_prim_path: str = self._builder.add_variable_node(
            animation_graph_prim_path,
            self._root_rotation_displacement_variable_name,
        )
        read_blend_shape_weights_variable_node_prim_path: str = self._builder.add_variable_node(
            animation_graph_prim_path, self._blend_shape_weights_variable_name
        )
        read_blinking_strength_variable_node_prim_path: str = self._builder.add_variable_node(
            animation_graph_prim_path, self._blinking_strength_variable_name
        )
        read_blend_shape_strength_variable_node_prim_path: str = self._builder.add_variable_node(
        ...
        self._builder.connect_nodes(blend_blinking_darting_node_prim_path, filter_node_prim_path, "inputs:pose")

        # Connect main blend node inputs
        # TODO: Disconnect the automatic connection first or let this override it.
        self._builder.connect_nodes(main_state_machine_prim_path, blend_node_prim_path, "inputs:pose0")
        self._builder.connect_nodes(filter_node_prim_path, blend_node_prim_path, "inputs:pose1")

        # Connect the animation graph output
        self._builder.connect_nodes(blend_node_prim_path, animation_graph_prim_path, "inputs:pose")

        print_or_log("======================= ANIMATION GRAPH GENERATION DONE =======================")
    ...
    def on_update(self, current_time: float, delta_time: float) -> None:
        ...
        all_skel_root_parent_parent_child_prims = self._stage.GetPrimAtPath(
            skel_root_parent_parent_prim_path
        ).GetAllChildren()
        ...
        all_skel_root_prim_paths: List[str] = [
            str(prim.GetPath()) + "/" + skel_root_prim_name for prim in all_skel_root_parent_parent_child_prims
        ]

        # Remove unused states
        self._skel_root_states = {
            skel_root_prim_path: skel_root_state
            for skel_root_prim_path, skel_root_state in self._skel_root_states.items()
            if skel_root_prim_path in all_skel_root_prim_paths
        }
        ...
        for prim in all_skel_root_parent_parent_child_prims:
            ...
            skel_root_prim_path: str = (
                skel_root_parent_parent_prim_path.pathString
                + "/"
                + skel_root_parent_prim_name
                + "/"
                + skel_root_prim_name
            )
        ...
        assert len(self._skel_root_states) == len(
            all_skel_root_parent_parent_child_prims
        ), f"{len(self._skel_root_states)} == {len(all_skel_root_parent_parent_child_prims)}"

    def update_with_skel_root(
        self, skel_root_prim_path: str, skel_root_state: SkelRootState, current_time: float
    ) -> None:
        ...
        if skel_root_state.initialization_count < 2:
            ...
            character.set_variable(
                ...
                self._posture_state_variable_name,
                self._config.default_posture_name.lower()
        ...
        for posture in self._postures:
            ...
            if character.is_node_active(
                ...
                skel_root_state.variant_state_variable_value = (skel_root_state.variant_state_variable_value + 1) % len(
                    posture.prim_variants
                )
                print_or_log_stream(
                    f"Variant state variable value: {skel_root_state.variant_state_variable_value} | Posture: {posture.name}"
                )
                character.set_variable(
                    self._variant_state_variable_name,
                    str(skel_root_state.variant_state_variable_value),
